...
public class Zscene implements Screen {
	...
	public void render(float delta) {
		...
		zenemy.updateAI(zplayer);

		// Vector3 tmp1 = new Vector3(zplayer.worldpos.x,
		// zplayer.worldpos.y, 0);
		// camera.project(tmp1);

		// zenemy.update(tmp1.x-27f, tmp1.y, true, camera,
		// true);

		//zenemy.update(zenemy.worldpos.x, zenemy.worldpos.y, true, camera,
			//	true);
		
		zenemy.update(true, camera,	true);
		
		// zenemy.update(-1, 0, true, camera,
		// true);
		...
		/*
		 * zplayer.update(zenemy.worldpos.x, zenemy.worldpos.y, true, camera,
		 * playerShoot);
		 */
		...
		if (true) {
			...
			bulletArray[activeBullet].isPlayers = false;
			Vector3 tmpVec3 = new Vector3(zenemy.worldpos.x, zenemy.worldpos.y,
					0);
			bulletArray[activeBullet].screenpos.y = zenemy.worldpos.y;
			bulletArray[activeBullet].screenpos.x = zenemy.worldpos.x;
		...
		for (int i = 0; i < 100; i++) {
			if (bulletArray[i].isAlive) {
				...
				if (bulletArray[i].isPlayers) {
					if ((Math.abs(bulletArray[i].screenpos.x
							- zenemy.worldpos.x) < 1)) {
						if (Math.abs(bulletArray[i].screenpos.y
								- zenemy.worldpos.y) < 1) {
							zenemy.goJump();
						}
					}
		...
		zenemy.sprite.draw(batch);
		if (!zenemy.isOnLadder) {
			zenemy.armsprite.draw(batch);
		}
		if (zenemy.isOnLadder && zenemy.isShooting) {
			zenemy.armsprite.draw(batch);
		}
