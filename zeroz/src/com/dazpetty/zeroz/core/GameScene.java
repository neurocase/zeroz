...
public class GameScene implements Screen {
	...
	public GameScene(final ZerozGame gam) {
		...
		TiledLayerManager tm = new TiledLayerManager();
			...
			for (int w = 0; w < tm.collisionLayer.getWidth(); w++) {
				...
				if (tm.isCellDestroyable(w, h)) {
					...
					if (TOTAL_DESTROYABLES < DESTROYABLE_LIMIT) {
						destroyable[value] = new Destroyable(w, h,
								value, world);
				...
				if (tm.isCellItem(w, h)){
					if (TOTAL_ITEMS < ITEM_LIMIT) {
						...
						TOTAL_ITEMS++;
						System.out.println( value + " ITEM ADDED: " + TOTAL_ITEMS + "at:" + w + "," + h);
					}
				}
			}
		}
	...
	private void createActor(int s, Vector2 nstartpos) {
		...
		if (s == 2) {
			...
			if (timenow - lasttimespawn > (50 * 50)
				...
				if (zenemy[enemycount] != null
					...
					System.out.println("Spawning New Enemy:" + enemycount
							+ " X:" + startpos.x + "Y" + startpos.y);
					/*
					 *  I HAVE NO IDEA WHY I NEED TO PASS zplayer.tm INSTEAD OF tm, BUT IT WONT WORK 
					 *  OTHERWISE.
					 */
					zenemy[enemycount] = new Actor(camera, world, true, zplayer.tm,
							startpos, enemycount, humanSprite, aiProjMan);
				}
			...
			if (projMan.proj[i] != null) {
				if (projMan.proj[i].isAlive) {
					if (projMan.proj[i].isAlive) {
						...
						projMan.proj[i].projsprite.setPosition(tmpprojpos.x - 0.25f,
								tmpprojpos.y - 0.25f);
	...
	public void render(float delta) {
		 ...
		 *  UPDATE PLAYER
		 */
		zplayer.update(inputHandler.giveWorldPos, camera,
				playerShoot);
		/*
		...
		for (int i = 0; i < aiprojbodies.size; i++) {
			...
			System.out.println("Destroying :" + b.getUserData());
			aiProjMan.KillProjectile(Integer.parseInt((String) b.getUserData()));
			if (cl.DamagePlayer()){
		...
		for (int i = 0; i < destroybodies.size; i++) {
			...
			int t = Integer.parseInt((String) b.getUserData());
			System.out.println("Destroying Destroyable:" + b.getUserData() + "at Address:" + t);
			...
			zplayer.tm.keys[killKeyValue] = true;
				for (int j = 0; j < DOOR_LIMIT; j++) {
					...
					System.out.println("is NULL, destroyable.length =" + destroyable.length);
				}
		}
		destroybodies.clear();
